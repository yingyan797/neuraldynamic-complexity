...
class SWMNetwork:
    def __init__(self, EE_module_neurons=100, EE_module_edges=1000, EE_module_num=8, i_neuron_num=200, p=0.1, dmax=20):
        ...
        EI_blocks = [self.create_EI_block(i, self.ee_m_neurons, self.i_neurons) * F_EI for i in range(0, self.i_neurons, 25)]
        ...
        ## generate W matrix
        W = np.bmat([
            [zero_block for _ in range(0, i)] + [self.create_EE_block(self.ee_m_neurons, self.ee_m_edges) * F_EE] + [zero_block for _ in range(i+1, self.modules_num)] + [EI_blocks[i]] for i in range(self.modules_num)
        ] + [IE_blocks + [II_block]])

        self.plot_weight_matrix(W)
        ...
        self._rewire(W, p)
        self.plot_weight_matrix(W, "static/weight_rewired.png")
    ...
    def simulate(self, period=1000, window_length=50):
        ...
        for t in range(period):
            ...
            for i in filter(lambda n: n < self.modules_num*self.ee_m_neurons, self.net.update()):
                fire_time.append(t)
                fire_num.append(i)
                module = i // self.ee_m_neurons
                spike_counts[module, window] += 1
        spike_counts /= window_length / period
